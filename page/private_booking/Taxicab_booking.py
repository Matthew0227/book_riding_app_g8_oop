...
from backend.fare_calculator import Taxicab

# === SESSION/DISCOUNT STATE ===
session_discount = discount
# === MAIN TKINTER WINDOW ===
root = tk.Tk()
root.title("Taxicab Booking System")
...
def book_ride():
    ...
    messagebox.showinfo("Booking Confirmed", f"Taxicab booked from {start} to {end}. Fare: ₱{fare}")
    fare_label.config(text=f"Estimated Fare: ₱{fare}")
