...
from backend.fare_calculator import Motorcycle

# === SESSION/DISCOUNT STATE ===
session_discount = discount.lower()

# === MAIN TKINTER WINDOW ===
root = tk.Tk()
root.title("Mototaxi Booking System")
...
def calculate_and_display_fare(start, end, discount):
    ...
    moto = Motorcycle(discount)
    fare = moto.calculate_fare(distance)
...
def book_ride():
    ...
    messagebox.showinfo("Booking Confirmed", f"Mototaxi booked from {start} to {end}. Fare: ₱{fare}")
    fare_label.config(text=f"Estimated Fare: ₱{fare}")
